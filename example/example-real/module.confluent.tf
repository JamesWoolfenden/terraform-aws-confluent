module "cluster" {
  ...
  bastion_subnet                = tolist(data.aws_subnets.public.ids)[0]
  broker_subnets                = data.aws_subnets.private.ids
  ...
  consumer_subnets              = tolist(data.aws_subnets.private.ids)
  control_center_private_ip     = local.control_center_private_ip
  control_center_subnets        = tolist(data.aws_subnets.private.ids)
  domain                        = var.domain
  key_name                      = "id_rsa.${var.account_name}"
  name                          = var.cluster_name
  producer_subnets              = [data.aws_subnets.private.ids, data.aws_subnets.private.ids]
  private_zone                  = data.aws_route53_zone.selected
  schema_private_ip             = local.schema_private_ip
  private_subnets               = tolist(data.aws_subnets.private.ids)
  stunnel_cert                  = tls_private_key.example.private_key_pem
  vpc_cidr                      = data.aws_vpc.vpc.cidr_block
  vpc_id                        = local.vpc_id
  zk_private_ip                 = local.zk_private_ip
  zk_subnets                    = concat(tolist(data.aws_subnets.private.ids), tolist(data.aws_subnets.private.ids))
}
