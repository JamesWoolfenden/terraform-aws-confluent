...
aws s3 --region ${region} cp s3://certificates-${aws_id}/confluent.schema-registry/$(hostname) /var/ssl/private --recursive
chown -R cp-schema-registry:confluent /var/ssl/private/*

mkdir -p /etc/security/keytabs
aws s3 --region ${region} cp s3://kerberos-${aws_id}/schema${count}.${account_name}.keytab /etc/security/keytabs/schema.service.keytab
